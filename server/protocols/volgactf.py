...
class API:
    ...
    def parse_flag_info_response(self, flag: str, response: requests.Response):
        if response.status_code == 200:
            ...
            if self.flag_is_fresh(info):
                return True, None
            return False, SubmitResult(flag, Flag_Status.REJECTED, 'expired')
        ...
        if respcode == GetInfoResult.ERROR_RATELIMIT:
            return False, SubmitResult(flag, Flag_Status.QUEUED, 'flag info ratelimit')

        return False, SubmitResult(flag, Flag_Status.QUEUED, f'error response from flag getinfo: {respcode}')
    ...
    def parse_flag_submit_response(flag: str, response: requests.Response):
        ...
        return SubmitResult(flag, Flag_Status.QUEUED, f'unknown checksystem code: {result_code}')
...
def submit_flags(flags:list[Flag_Model], config:Config_Model):
    ...
    queued = map(
        lambda f: SubmitResult(f, Flag_Status.QUEUED, 'flag submission ratelimit'),
